...
foreach ($required in @("COUCHBASE_CONN_STR", "COUCHBASE_USERNAME", "COUCHBASE_PASSWORD")) {
...
$env:TF_VAR_couchbase_conn_str = $envVars["COUCHBASE_CONN_STR"]
$seedNodes = $envVars["COUCHBASE_SEED_NODES"]
if (-not $seedNodes) {
    $seedNodes = $envVars["COUCHBASE_CONN_STR"] -replace '^couchbases?://', ''
    $seedNodes = $seedNodes -replace '/.*$', ''
}
$env:TF_VAR_couchbase_seed_nodes = $seedNodes
$env:TF_VAR_couchbase_username = $envVars["COUCHBASE_USERNAME"]
$env:TF_VAR_couchbase_password = $envVars["COUCHBASE_PASSWORD"]
...
Write-Host "---------------------------------------------------------" -ForegroundColor Green
Write-Host "Wait a few minutes for cloud-init to finish configuring the VM."
